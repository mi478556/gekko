...
def run_dummy_training(status_callback=None, strategy='ppo', start_date='2020-01-01', end_date='2020-12-31', capital=1e6):
    ...
    try:
        ...
        model = agent.get_model(strategy)
        if device == "cuda":
            model.policy.to(device)
        total_steps = 2048
        # Use ProgressCallback for real-time progress updates
        progress_callback = ProgressCallback(status_callback, total_steps)
        model.learn(
        ...
        if hasattr(model, 'env') and hasattr(model.env, 'envs'):
            ...
            final_portfolio_value = env.asset_memory[-1] if hasattr(env, 'asset_memory') else None
            total_trades = env.trades if hasattr(env, 'trades') else None
            returns = (env.asset_memory[-1] - env.asset_memory[0]) / env.asset_memory[0] if hasattr(env, 'asset_memory') else None
            sharpe = None
